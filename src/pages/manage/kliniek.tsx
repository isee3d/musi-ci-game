...
const ManageKliniek: NextPage = () => {
  ...
  } = useForm<Kliniek>({ mode: 'onBlur' })
  const kliniekQuery = api.kliniek.getAllKlinieks.useQuery()
  const ctx = api.useContext()
  const { mutate: addKliniek } = api.kliniek.createKliniek.useMutation({
    onSuccess: () => {
      ctx.kliniek.getAllKlinieks.invalidate()
    },
  })
      ...
      <main className="flex grow flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
        ...
        <div className="container mx-auto flex w-1/2 flex-col items-center rounded border-2 border-white p-4 shadow ">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            >
              <h3 className="text-center text-2xl font-bold">Nieuwe Kliniek opslaan</h3>
            </button>
          </form>
        </div>
      </main>
