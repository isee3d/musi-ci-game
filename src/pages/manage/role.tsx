...
const ManageRole = () => {
  ...
  } = useForm<Role>({ mode: 'onBlur' })
  const roleQuery = api.role.getAllRoles.useQuery()
  const ctx = api.useContext()
  const { mutate: addKliniek } = api.role.createRole.useMutation({
    onSuccess: () => {
      ctx.role.getAllRoles.invalidate()
    },
  })

  const onSubmit: SubmitHandler<Role> = (data) => {
    const exists = roleQuery.data?.find((role) => role.name === data.name)
    const toastMessage = exists ? 'Role already exists!' : 'Role created!'
      ...
      <main className="flex grow flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
        ...
        <div className="container mx-auto flex w-1/2 flex-col items-center rounded border-2 border-white p-4 shadow ">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            >
              <h3 className="text-center text-2xl font-bold">Nieuwe Role opslaan</h3>
            </button>
          </form>
        </div>
      </main>
