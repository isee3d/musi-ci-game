...
const ManageGame: NextPage = () => {
    const { register, handleSubmit, reset, formState: { errors, isValid } } = useForm<Game>({ mode: 'onBlur' });
    const gameQuery = api.game.getAllGames.useQuery();
    const ctx = api.useContext();
    const { mutate: addGame } = api.game.createGame.useMutation(
        {
            onSuccess: () => {
    ...
    const onSubmit: SubmitHandler<Game> = (data) => {
        const exists = gameQuery.data?.find((game) => game.name === data.name);
        const toastMessage = exists ? "Game already exists!" : "game created!";
        exists ? toast.error(toastMessage) : (addGame(data), toast.success(toastMessage));
        reset();
    };
            ...
            <main className="flex grow flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
                ...
                <div className="container mx-auto flex w-1/2 flex-col items-center rounded border-2 border-white p-4 shadow ">
                    <form onSubmit={ handleSubmit(onSubmit) }>
                        ...
                        >
                            <h3 className="text-center text-2xl font-bold">Nieuwe game opslaan</h3>
                        </button>
                    </form>
